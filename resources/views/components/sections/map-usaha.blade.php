<section class="gmaps-section" id="peta-usaha">
    <div class="gmaps-section__header">
        <span class="gmaps-section__eyebrow">Peta Interaktif</span>
        <h2 class="gmaps-section__title">Persebaran Lokasi Usaha Hasil Scraping Google Maps</h2>
        <p class="gmaps-section__description">
    ...
    <div class="gmaps-section__container">
        <div class="gmaps-stats-row">
            <div class="gmaps-stat-card gmaps-stat-card--total">
                <div class="gmaps-stat-card__icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"></path>
                        <circle cx="12" cy="10" r="3"></circle>
                    </svg>
                </div>
                <div class="gmaps-stat-card__content">
                    <span class="gmaps-stat-card__value">{{ $statsGmaps['total'] }}</span>
                    <span class="gmaps-stat-card__label">Total Usaha</span>
                </div>
            </div>

            <div class="gmaps-stat-card gmaps-stat-card--sbr">
                <div class="gmaps-stat-card__icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="gmaps-stat-card__content">
                    <span class="gmaps-stat-card__value">{{ $statsGmaps['dalam_sbr'] }}</span>
                    <span class="gmaps-stat-card__label">Dalam SBR</span>
                </div>
            </div>

            <div class="gmaps-stat-card gmaps-stat-card--outside">
                <div class="gmaps-stat-card__icon">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="gmaps-stat-card__content">
                    <span class="gmaps-stat-card__value">{{ $statsGmaps['luar_sbr'] }}</span>
                    <span class="gmaps-stat-card__label">Luar SBR</span>
                </div>
            </div>
        </div>

        <div class="gmaps-section__legend">
            <div class="gmaps-legend">
                <span class="gmaps-legend__title">Keterangan:</span>
                <div class="gmaps-legend__items">
                    <div class="gmaps-legend__item">
                        <span class="gmaps-legend__marker gmaps-legend__marker--sbr"></span>
                        <span>Dalam SBR</span>
                    </div>
                    <div class="gmaps-legend__item">
                        <span class="gmaps-legend__marker gmaps-legend__marker--outside"></span>
                        <span>Luar SBR</span>
                    </div>
                </div>
            </div>
        ...
        <div class="gmaps-section__wrapper">
            <div id="usaha-gmaps" class="usaha-gmaps"></div>
        </div>

        <div class="gmaps-table-section">
            <div class="gmaps-table-section__header">
                <h3 class="gmaps-table-section__title">Daftar Usaha Hasil Scraping Google Maps</h3>
                <div class="gmaps-table-section__controls">
                    <div class="gmaps-table-search">
            ...
            <div id="pagination-wrapper">
                @include('components.partials.gmaps-table-pagination', ['allUsahaGmaps' => $allUsahaGmaps])
            </div>
        </div>
    </div>
</section>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (usahaData.length === 0) {
        document.getElementById('usaha-gmaps').innerHTML = '<p style="text-align:center;padding:40px;color:#696969;">Tidak ada data lokasi usaha</p>';
    } else {
        const map = L.map('usaha-gmaps').setView([2.7307908972918296, 125.40917238648026], 14);
            ...
            if (usaha.latitude && usaha.longitude) {
                ...
                let popupContent = `
                    <div style="min-width: 200px;">
                        ...
                        <p style="margin: 8px 0 0 0; font-size: 12px; font-weight: 600; padding: 4px 8px; border-radius: 4px; display: inline-block; ${usaha.is_in_sbr ? 'background-color: #00a6fb; color: white;' : 'background-color: red; color: white;'}">${status}</p>
                    </div>
                `;
                ...
                L.marker([usaha.latitude, usaha.longitude], { icon: icon })
                    ...
                    .bindTooltip(
                        ...
                        `<span style="color:#ea782c;">${usaha.kategori || '-'}</span><br/>` +
    ...
    function filterTable() {
        ...
        rows.forEach(row => {
            const name = row.querySelector('.gmaps-table__name').textContent.toLowerCase();
            const kategori = row.querySelector('.gmaps-table__badge').textContent.toLowerCase();
            const address = row.querySelector('.gmaps-table__address').textContent.toLowerCase();
