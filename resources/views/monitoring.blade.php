<x-layouts.app>
    ...
    <section class="monitoring-section">
        <div class="monitoring-section__header">
            <span class="monitoring-section__eyebrow">Monitoring</span>
        ...
        <div class="monitoring-section__container">
            ...
            <div class="monitoring-table-section">
                <h3 class="monitoring-table-section__title">Detail Progress per Kecamatan</h3>
                    ...
                    <table class="monitoring-table">
                        ...
                        <tbody>
                            @foreach($progressData as $kecamatan => $data)
                                <tr>
                                    <td class="monitoring-table__kecamatan">{{ $kecamatan }}</td>
                                    <td>
                                        @php
                                            $barColor = $data['progress'] == 100 ? '#10B981' : ($data['progress'] >= 71 ? '#00a6fb' : ($data['progress'] >= 41 ? '#F59E0B' : '#EF4444'));
                                        @endphp
                                        <div class="monitoring-progress">
                                            <div class="monitoring-progress__bar">
                                                <div class="monitoring-progress__fill" style="width: {{ $data['progress'] }}%; background-color: {{ $barColor }}"></div>
                                            </div>
                                            <span class="monitoring-progress__value">{{ $data['progress'] }}%</span>
                                        </div>
                                    </td>
                                    <td class="monitoring-table__completed">{{ number_format($data['completed']) }}</td>
                                    <td class="monitoring-table__target">{{ number_format($data['target']) }}</td>
                                    <td>
                                        @if($data['progress'] == 100)
                                            <span class="monitoring-status monitoring-status--complete">Selesai</span>
                                        @elseif($data['progress'] >= 71)
                                            <span class="monitoring-status monitoring-status--high">Tinggi</span>
                                        @elseif($data['progress'] >= 41)
                                            <span class="monitoring-status monitoring-status--medium">Sedang</span>
                                        @else
                                            <span class="monitoring-status monitoring-status--low">Rendah</span>
                                        @endif
                                    </td>
                                </tr>
                            @endforeach
